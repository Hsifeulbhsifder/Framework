...
public abstract class StateMachineSubsystemBase extends SubsystemBase {
  ...
  public void setCurrentState(State state) {
    if (state.subsystem != this) {
      System.err.printf("[SME] Cannot set the state of <%s> to <%s>.<%s>\n", this.getName(),
          state.subsystem.getName(), state.name);
    }
  ...
  public final void periodic() {
    ...
    outputPeriodic();
    Logger.getInstance().recordOutput(this.getName() + "/PerfDelta", Logger.getInstance().getRealTimestamp() - time);
